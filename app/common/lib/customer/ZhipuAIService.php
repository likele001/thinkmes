...
class ZhipuAIService
{
    ...
    public function __construct()
    {
        ...
        $this->systemPrompt = $config['system_prompt'] ?? '你是一个专业的客服助手，帮助用户解答关于ThinkMES系统的问题。';
        
        if (empty($this->apiKey)) {
            throw new \Exception('智谱AI API密钥未配置');
        }
    }
     ...
     * 获取智谱AI配置
     */
    private function getZhipuAIConfig(): array
    {
        $config = Db::name('cs_config')
            ->where('config_key', 'zhipuai_settings')
            ->where('tenant_id', 0)
            ->value('config_value');
     ...
     * 生成JWT Token
     */
    private function generateToken(string $apiKey, int $expSeconds = 3600): string
    {
    ...
    private function getKnowledgeBaseContext(string $question, int $tenantId = 0): array
    {
        ...
        $articles = Db::name('cs_kb_article')
            ...
            ->field('a.id,a.title,a.summary,c.name as category_name')
            ->limit(3)
            ->select()
            ->toArray();
     ...
     * 调用智谱AI API
     */
    private function callZhipuAI(array $messages): array
    {
        try {
            $token = $this->generateToken($this->apiKey);
            ...
            if (isset($result['error'])) {
                throw new \Exception($result['error']['message'] ?? 'API错误');
            }
    ...
    {
        // 1. 检查缓存
        $cacheKey = 'zhipu_answer:' . md5($question);
        ...
        // 5. 调用智谱AI API
        $result = $this->callZhipuAI($messages);
    ...
    {
        return [
            'glm-4-flash' => 'GLM-4 Flash (免费，速度快)',
            'glm-4' => 'GLM-4 (付费，能力强)',
            'glm-4-plus' => 'GLM-4 Plus (付费，能力最强)',
            'glm-3-turbo' => 'GLM-3 Turbo (免费)',
        ];
    }
}
