...
use app\admin\model\hr\HrOvertimeModel;
use app\admin\model\hr\HrEmployeeModel;
use think\facade\View;
use think\Response;

class Overtime extends Backend
{
    public function index(): string|Response
    {
        $limitParam = $this->request->get('limit');
        if (!$this->request->isAjax() || $limitParam === null || $limitParam === '') {
            View::assign('title', '加班管理');
            return $this->fetchWithLayout('hr/overtime/index');
        }
        $limit = max(1, min(100, (int) $this->request->get('limit', 20)));
        $offset = $this->request->get('offset');
        $page = $offset !== null && $offset !== '' ? (int) floor((int) $offset / $limit) + 1 : max(1, (int) $this->request->get('page', 1));
        $tenantId = $this->getTenantId();
        $query = HrOvertimeModel::with(['employee'])->order('id', 'desc');
        if ($tenantId > 0) {
        ...
        foreach ($list as &$item) {
            ...
            $item['status_text'] = (HrOvertimeModel::getStatusList())[$item['status']] ?? '';
        }
        unset($item);
    ...
    public function add(): string|Response
    {
        if ($this->request->isPost()) {
            ...
            if (empty($params) || empty($params['employee_id']) || empty($params['work_date']) || empty($params['hours'])) {
                return $this->error('请填写员工、日期与时长');
            }
            $params['tenant_id'] = $this->getTenantId();
            $params['create_time'] = time();
            $params['update_time'] = time();
            try {
                $row = HrOvertimeModel::create($params);
        ...
        View::assign('statusList', HrOvertimeModel::getStatusList());
        View::assign('title', '添加加班');
        return $this->fetchWithLayout('hr/overtime/add');
    }
    ...
    public function edit(): string|Response
    {
        ...
        $tenantId = $this->getTenantId();
        $row = HrOvertimeModel::where('tenant_id', $tenantId)->find($ids);
        if (!$row) {
        ...
        View::assign('statusList', HrOvertimeModel::getStatusList());
        View::assign('row', $row);
        View::assign('title', '编辑加班');
        return $this->fetchWithLayout('hr/overtime/edit');
    }
    ...
    public function del(): Response
    {
        ...
        foreach ($idsArr as $id) {
            $row = HrOvertimeModel::where('tenant_id', $tenantId)->find($id);
            if ($row) {
                $row->delete();
                $count++;
            }
