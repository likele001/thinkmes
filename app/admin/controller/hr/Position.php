...
class Position extends Backend
{
    public function index(): string|Response
    {
        ...
        if (!$this->request->isAjax() || $limitParam === null || $limitParam === '') {
            View::assign('title', '岗位管理');
            return $this->fetchWithLayout('hr/position/index');
        }
        $limit = max(1, min(100, (int) $this->request->get('limit', 20)));
        $page = max(1, (int) $this->request->get('page', 1));
        $tenantId = $this->getTenantId();
        $query = HrPositionModel::order('sort', 'asc')->order('id', 'asc');
        if ($tenantId > 0) {
    ...
    public function add(): string|Response
    {
        if ($this->request->isPost()) {
            ...
            if (empty($params) || empty($params['name'])) {
                return $this->error('岗位名称不能为空');
            }
            $params['tenant_id'] = $this->getTenantId();
            $params['create_time'] = time();
            $params['update_time'] = time();
            try {
                $row = HrPositionModel::create($params);
                return $this->success('添加成功', ['id' => $row->id]);
            } catch (\Throwable $e) {
                return $this->error('添加失败：' . $e->getMessage());
            }
        }
        View::assign('title', '添加岗位');
        return $this->fetchWithLayout('hr/position/add');
    }
    ...
    public function edit(): string|Response
    {
        ...
        $tenantId = $this->getTenantId();
        $row = HrPositionModel::where('tenant_id', $tenantId)->find($ids);
        if (!$row) {
        ...
        View::assign('row', $row);
        View::assign('title', '编辑岗位');
        return $this->fetchWithLayout('hr/position/edit');
    }
    ...
    public function del(): Response
    {
        ...
        foreach ($idsArr as $id) {
            $row = HrPositionModel::where('tenant_id', $tenantId)->find($id);
            if ($row) {
                $row->delete();
                $count++;
            }
