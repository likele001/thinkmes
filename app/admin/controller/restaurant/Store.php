...
class Store extends Backend
{
    public function index(): string|Response
    {
        ...
        if (!$this->request->isAjax() && ($limitParam === null || $limitParam === '')) {
            View::assign('title', '门店管理');
            return $this->fetchWithLayout('restaurant/store/index');
        }
        ...
        $query = StoreModel::order('id', 'desc');
        if ($tenantId > 0) {
    ...
    public function add(): string|Response
    {
        if ($this->request->isPost()) {
            ...
            try {
                $row = StoreModel::create($params);
                return $this->success('添加成功', ['id' => $row->id]);
            } catch (\Throwable $e) {
                return $this->error('添加失败：' . $e->getMessage());
            }
        }
        View::assign('title', '添加门店');
        return $this->fetchWithLayout('restaurant/store/add');
    }
    ...
    public function edit(): string|Response
    {
        ...
        $tenantId = $this->getTenantId();
        $row = StoreModel::where('tenant_id', $tenantId)->find($ids);
        if (!$row) {
        ...
        View::assign('row', $row);
        View::assign('title', '编辑门店');
        return $this->fetchWithLayout('restaurant/store/edit');
    }
    ...
    public function del(): Response
    {
        ...
        foreach ($idsArr as $id) {
            $row = StoreModel::where('tenant_id', $tenantId)->find($id);
            if ($row) {
                $row->delete();
                $count++;
            }
