...
class Category extends Backend
{
    public function index(): string|Response
    {
        ...
        if (!$this->request->isAjax() && ($limitParam === null || $limitParam === '')) {
            ...
            View::assign('title', '菜品分类');
            return $this->fetchWithLayout('restaurant/category/index');
        }
        ...
        $tenantId = $this->getTenantId();
        $query = CategoryModel::with(['store'])->order('sort', 'desc')->order('id', 'desc');
        if ($tenantId > 0) {
    ...
    public function add(): string|Response
    {
        ...
        if ($this->request->isPost()) {
            ...
            try {
                $row = CategoryModel::create($params);
        ...
        return $this->fetchWithLayout('restaurant/category/add');
    }
    ...
    public function edit(): string|Response
    {
        ...
        $tenantId = $this->getTenantId();
        $row = CategoryModel::where('tenant_id', $tenantId)->find($ids);
        if (!$row) {
        ...
        return $this->fetchWithLayout('restaurant/category/edit');
    }
    ...
    public function del(): Response
    {
        ...
        foreach ($idsArr as $id) {
            $row = CategoryModel::where('tenant_id', $tenantId)->find($id);
            if ($row) {
                $row->delete();
                $count++;
            }
