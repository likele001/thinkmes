-- MES 新增模块菜单配置
-- 执行时间: 2026-02-11
-- 使用 INSERT IGNORE，重复执行不会报 Duplicate entry，已存在则跳过
...
-- 1. 采购管理模块菜单
-- ----------------------------
INSERT IGNORE INTO `fa_auth_rule` (`pid`, `name`, `title`, `icon`, `type`, `status`, `sort`, `create_time`, `update_time`) VALUES
...
INSERT IGNORE INTO `fa_auth_rule` (`pid`, `name`, `title`, `type`, `status`, `sort`, `create_time`, `update_time`) VALUES
...
(@stock_pid, 'mes/stock/log', '物料流水', 1, 1, 3, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
(@stock_pid, 'mes/stock/product_log', '产品流水', 1, 1, 4, UNIX_TIMESTAMP(), UNIX_TIMESTAMP());
