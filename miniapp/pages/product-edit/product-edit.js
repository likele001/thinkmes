...
Page({
    ...
    if (this.data.id) {
      adminApi.getProducts(1, 200).then((res) => {
        ...
        this.setData({
          name: d.name || '',
          code: d.code || '',
          description: d.description || d.specification || '',
          remark: d.remark || '',
          status: st,
  ...
  submit() {
    const { id, name, code, description, remark, status } = this.data;
    if (!name || !name.trim()) { wx.showToast({ title: '请输入产品名称', icon: 'none' }); return; }
    this.setData({ loading: true });
    const row = { name: name.trim(), code: (code || '').trim(), description: (description || '').trim(), remark: (remark || '').trim(), status };
    const p = id ? adminApi.updateProduct({ id, row, models: [], prices: [] }) : adminApi.createProduct({ row, models: [], prices: [] });
    p.then(() => {
