...
Page({
  ...
  load() {
    userApi.getTaskInfo(this.data.allocationId)
      .then((res) => this.setData({ info: res.data }))
      .catch(() => {});
  },
  ...
  submit() {
    ...
    const uploads = (images.length ? Promise.all(images.map(path => userApi.uploadImage(path))) : Promise.resolve([]))
      .then((urls) => {
        ...
        return userApi.submitReport(data);
      });
    uploads
      .then(() => {
