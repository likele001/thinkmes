...
Page({
  ...
  load() {
    ...
    adminApi.getMaterialList(this.data.page, this.data.limit)
      .then((res) => {
        const d = res.data || {};
  ...
  confirmDelete(e) {
    ...
    if (!id) return;
    wx.showModal({ title: '确认删除', content: '确定删除该物料？', success: (res) => {
      if (res.confirm) adminApi.deleteMaterial(id).then(() => { wx.showToast({ title: '已删除' }); this.setData({ page: 1, list: [], noMore: false }); this.load(); }).catch(() => {});
    }});
  },
});
