...
Page({
  ...
  goDetail(e) { const id = e.currentTarget.dataset.id; if (id) wx.navigateTo({ url: '/pages/product-edit/product-edit?id=' + id }); },
  goEdit(e) { const id = e.currentTarget.dataset.id; if (id) wx.navigateTo({ url: '/pages/product-edit/product-edit?id=' + id }); },
  goModels(e) { const id = e.currentTarget.dataset.id; if (id) wx.navigateTo({ url: '/pages/product-batch-models/product-batch-models?product_id=' + id }); },
  confirmDelete(e) {
    const id = e.currentTarget.dataset.id;
    if (!id) return;
    wx.showModal({ title: '确认删除', content: '确定删除该产品？', success: (res) => {
      if (res.confirm) adminApi.deleteProduct(id).then(() => { wx.showToast({ title: '已删除' }); this.setData({ page: 1, list: [], noMore: false }); this.load(); }).catch(() => {});
    }});
  },
});
