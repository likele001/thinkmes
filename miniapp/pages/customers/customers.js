...
Page({
  ...
  onLoad() { this.load(); },
  onShow() { if (this.data.list.length > 0) this.load(); },
  load() {
    if (this.data.loading || this.data.noMore) return;
    this.setData({ loading: true });
    adminApi.getCustomerList(this.data.page, this.data.limit)
      .then((res) => {
        const d = res.data || {};
  ...
  confirmDelete(e) {
    ...
    if (!id) return;
    wx.showModal({ title: '确认删除', content: '确定删除该客户？', success: (res) => {
      if (res.confirm) adminApi.deleteCustomer(id).then(() => { wx.showToast({ title: '已删除' }); this.setData({ page: 1, list: [], noMore: false }); this.load(); }).catch(() => {});
    }});
  },
});
