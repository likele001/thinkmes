...
class {$TableName} extends Backend
{
    public function index(): string|Response
    {
        ...
        $query = {$TableName}Model::where('tenant_id', $tenantId)->order('id', 'desc');
        $total = $query->count();
    ...
    public function edit(): string|Response
    {
        ...
        $data = {$TableName}Model::where('tenant_id', $this->getTenantId())->find($id);
        if (!$data) {
    ...
    public function editPost(): Response
    {
        ...
        $row = {$TableName}Model::where('tenant_id', $this->getTenantId())->find($id);
        if (!$row) {
            return $this->error('记录不存在');
        }
        $data = $this->request->post();
