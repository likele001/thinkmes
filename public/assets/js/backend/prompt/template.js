    ...
    var Controller = {
        index: function () {
            ...
            table.bootstrapTable({
                url: base + '/prompt/template/index',
                method: 'get',
                ...
                columns: [
                    ...
                    { field: 'category', title: '分类', formatter: function (v) { return v && v.name ? v.name : '-'; } },
                    { field: 'description', title: '简介', formatter: function (v) { return v ? '<span title="' + v + '">' + v.substr(0, 30) + (v.length > 30 ? '...' : '') + '</span>' : '-'; } },
                    { field: 'use_count', title: '使用次数', width: 90 },
                    ...
                    { field: 'id', title: '操作', width: 140, formatter: function (v, row) {
                        return '<a href="' + base + '/prompt/template/edit?id=' + v + '" class="btn btn-xs btn-success">编辑</a> <a href="javascript:;" class="btn btn-xs btn-danger btn-del" data-id="' + v + '">删除</a>';
                    }}
                ]
            });
            $('#btn-search, #search-keyword').on('click keypress', function (e) { if (e.type === 'click' || e.which === 13) table.bootstrapTable('refresh'); });
            $('#filter-category').on('change', function () { table.bootstrapTable('refresh'); });
            $('#btn-add').on('click', function () {
                layer.open({ type: 2, title: '添加模板', area: ['820px', '90%'], content: base + '/prompt/template/add', end: function () { table.bootstrapTable('refresh'); } });
            });
            $(document).off('click', '.btn-del').on('click', '.btn-del', function () {
                var id = $(this).data('id');
                if (!confirm('确定删除该模板？')) return;
                $.post(base + '/prompt/template/del', { ids: id }, function (r) {
