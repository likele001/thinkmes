        ...
        index: function () {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    ...
                    { field: 'name', title: '模板名称', align: 'left' },
                    { field: 'type', title: '类型', width: 100 },
                    { field: 'operate', title: '操作', width: 140, formatter: function (v, row) {
                        return '<a href="' + base + '/print_template/edit?id=' + row.id + '" class="btn btn-xs btn-success">编辑</a> <a href="javascript:;" class="btn btn-xs btn-danger btn-del" data-id="' + row.id + '">删除</a>';
                    }}
                ]
            });
            ...
            $(document).off('click', '.btn-del').on('click', '.btn-del', function () {
                ...
                $.post(base + '/print_template/del', { ids: id }, function (r) { if (r.code == 1) { table.bootstrapTable('refresh'); alert(r.msg || '删除成功'); } else alert(r.msg || '删除失败'); }, 'json');
            });
        },
        add: function () {},
        edit: function () {}
