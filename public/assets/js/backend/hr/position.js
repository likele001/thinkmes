    ...
    var Controller = {
        index: function () {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    ...
                    { field: 'operate', title: '操作', width: 120, formatter: function (v, row) {
                        return '<a href="' + base + '/hr/position/edit?id=' + row.id + '" class="btn btn-xs btn-success">编辑</a> <a href="javascript:;" class="btn btn-xs btn-danger btn-del" data-id="' + row.id + '">删除</a>';
                    }}
                ]
            });
            ...
            $(document).off('click', '.btn-del').on('click', '.btn-del', function () {
                ...
                $.post(base + '/hr/position/del', { ids: id }, function (r) { if (r.code == 1) { table.bootstrapTable('refresh'); alert(r.msg || '删除成功'); } else alert(r.msg || '删除失败'); }, 'json');
            });
        },
        add: function () {
        ...
        edit: function () {
            ...
            form.off('submit').on('submit', function (e) {
                e.preventDefault();
                var id = form.data('id');
                $.post(base + '/hr/position/edit?ids=' + id, form.serialize(), function (r) {
                    if (r.code == 1) {
                        (window.layer ? layer.msg(r.msg || '保存成功', { icon: 1 }) : alert(r.msg || '保存成功'));
                        setTimeout(function () { location.href = base + '/hr/position/index'; }, 800);
                    } else {
