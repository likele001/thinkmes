...
(function () {
    ...
    var indexUrl = base + '/mes/product_model/index';
    var addUrl = base + '/mes/product_model/add';
    var editUrl = base + '/mes/product_model/edit';
    var delUrl = base + '/mes/product_model/del';
    ...
    var Controller = {
        index: function () {
            ...
            $table.bootstrapTable({
                ...
                columns: [
                    ...
                    {field: 'model_code', title: '型号编码', align: 'left', width: 100},
                    {field: 'color', title: '颜色', align: 'left', width: 80},
                    {field: 'specification', title: '规格', align: 'left', width: 100},
        ...
        api: {
            bindevent: function () {
                ...
                if (form.length) {
                    var isEdit = form.attr('id') === 'form-edit';
                    var action = form.attr('action') || (isEdit ? (base + '/mes/product_model/edit') : (base + '/mes/product_model/add'));
                    form.attr('action', action);
                    form.on('submit', function (e) {
                        e.preventDefault();
                        var url = $(this).attr('action');
                        // 编辑页面需要添加 id 参数
                        if (isEdit && url.indexOf('?') === -1) {
                        ...
                        $.post(url, $(this).serialize(), function (r) {
                            ...
                            if (r && r.code === 1) {
                                location.href = base + '/mes/product_model/index';
                            }
