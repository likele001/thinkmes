    ...
    var Controller = {
        index: function () {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    ...
                    { field: 'type', title: '跟进方式', width: 120, formatter: function (v) {
                        if (v === 'visit') return '<span class="badge badge-info">拜访</span>';
                        if (v === 'phone') return '<span class="badge badge-primary">电话</span>';
                        if (v === 'email') return '<span class="badge badge-success">邮件</span>';
                        return v || '';
                    }},
                    { field: 'content', title: '跟进内容', align: 'left', formatter: function (v) {
                        if (!v) return ''; return v.length > 50 ? v.substring(0, 50) + '...' : v;
                    }},
                    { field: 'next_follow_time', title: '下次跟进时间', width: 180, formatter: function (v) { return v ? new Date(v * 1000).toLocaleString('zh-CN') : ''; }},
                    ...
                    { field: 'operate', title: '操作', width: 120, events: {
                        'click .btn-edit': function (e, v, row) { location.href = base + '/crm/follow/edit?id=' + row.id; },
                        ...
                        return '<a href="' + base + '/crm/follow/edit?id=' + row.id + '" class="btn btn-xs btn-success btn-edit">编辑</a> <a href="javascript:;" class="btn btn-xs btn-danger btn-del">删除</a>';
                    }}
                ]
            });
            ...
            $(document).off('click', '#toolbar .btn-del').on('click', '#toolbar .btn-del', function () {
                ...
                $.post(base + '/crm/follow/del', { ids: rows.map(function (r) { return r.id; }).join(',') }, function (r) {
                    if (r.code == 1) { table.bootstrapTable('refresh'); alert(r.msg || '删除成功'); } else alert(r.msg || '删除失败');
                }, 'json');
            });
        ...
        edit: function () {
            ...
            form.off('submit').on('submit', function (e) {
                e.preventDefault();
                var id = form.data('id');
                $.post(base + '/crm/follow/edit?ids=' + id, form.serialize(), function (r) {
                    if (r.code == 1) {
                        alert(r.msg || '保存成功');
                        location.href = base + '/crm/follow/index';
                    } else {
                        alert(r.msg || '保存失败');
                    }
                }, 'json');
