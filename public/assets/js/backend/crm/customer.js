        ...
        index: function () {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    ...
                    { field: 'level', title: '级别', width: 100, formatter: function (v) {
                        if (v == 0) return '<span class="badge badge-secondary">普通</span>';
                        if (v == 1) return '<span class="badge badge-info">重要</span>';
                        if (v == 2) return '<span class="badge badge-warning">战略</span>';
                        return v;
                    }},
                    { field: 'source', title: '来源', align: 'left' },
                    { field: 'industry', title: '行业', align: 'left' },
                    { field: 'status', title: '状态', width: 100, formatter: function (v) {
                        return v == 1 ? '<span class="badge badge-success">正常</span>' : '<span class="badge badge-danger">停用</span>';
                    }},
                    ...
                    { field: 'operate', title: '操作', width: 120, events: {
                        'click .btn-edit': function (e, v, row) { location.href = base + '/crm/customer/edit?id=' + row.id; },
                        ...
                        return '<a href="' + base + '/crm/customer/edit?id=' + row.id + '" class="btn btn-xs btn-success btn-edit">编辑</a> <a href="javascript:;" class="btn btn-xs btn-danger btn-del">删除</a>';
                    }}
                ]
            });
            ...
            $(document).off('click', '#toolbar .btn-del').on('click', '#toolbar .btn-del', function () {
                ...
                $.post(base + '/crm/customer/del', { ids: rows.map(function (r) { return r.id; }).join(',') }, function (r) {
                    if (r.code == 1) { table.bootstrapTable('refresh'); alert(r.msg || '删除成功'); } else alert(r.msg || '删除失败');
                }, 'json');
            });
