    ...
    var Controller = {
        index: function () {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    ...
                    { field: 'amount', title: '回款金额', width: 120, formatter: function (v) {
                        return v ? parseFloat(v).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '';
                    }},
                    { field: 'pay_date', title: '回款日期', width: 150, formatter: function (v) {
                        return v ? new Date(v * 1000).toLocaleDateString('zh-CN') : '';
                    }},
                    { field: 'invoice_no', title: '发票号', align: 'left' },
                    ...
                    { field: 'operate', title: '操作', width: 120, events: {
                        'click .btn-edit': function (e, v, row) { location.href = base + '/crm/payment/edit?id=' + row.id; },
                        'click .btn-del': function (e, v, row) {
                            if (confirm('确定要删除吗？')) {
                                $.post(base + '/crm/payment/del', { ids: row.id }, function (r) {
                                    if (r.code == 1) { table.bootstrapTable('refresh'); alert(r.msg || '删除成功'); } else alert(r.msg || '删除失败');
                                }, 'json');
                            }
                        }
                    }, formatter: function (v, row) {
                        return '<a href="' + base + '/crm/payment/edit?id=' + row.id + '" class="btn btn-xs btn-success btn-edit">编辑</a> <a href="javascript:;" class="btn btn-xs btn-danger btn-del">删除</a>';
                    }}
                ]
            });
            ...
            $(document).off('click', '#toolbar .btn-edit').on('click', '#toolbar .btn-edit', function () {
                ...
                location.href = base + '/crm/payment/edit?id=' + rows[0].id;
            });
            $(document).off('click', '#toolbar .btn-del').on('click', '#toolbar .btn-del', function () {
                ...
                $.post(base + '/crm/payment/del', { ids: rows.map(function (r) { return r.id; }).join(',') }, function (r) {
                    if (r.code == 1) { table.bootstrapTable('refresh'); alert(r.msg || '删除成功'); } else alert(r.msg || '删除失败');
                }, 'json');
            });
        ...
        edit: function () {
            ...
            form.off('submit').on('submit', function (e) {
                e.preventDefault();
                var id = form.data('id');
                $.post(base + '/crm/payment/edit?ids=' + id, form.serialize(), function (r) {
                    if (r.code == 1) {
                        alert(r.msg || '保存成功');
                        location.href = base + '/crm/payment/index';
                    } else {
                        alert(r.msg || '保存失败');
                    }
                }, 'json');
